...
double noise = 1.00; // SET THIS TO -1 FOR NO NOISE, 0.1--> 0.05, 0.5-->0.25
...
double avg_exploration_time = 2800; //2800.0; //***--> time to be in exploration state--> fixed
double avg_uncommitted_time = 800.0; // time to stay in dissemination for uncommitted agents
double dissemparam = 1800.0; //1800
...
//    }
    ...
    if (currentopinion == 1) {
        ...
        set_color(RGB(0, 3, 0)); //uncomitted
    ...
    if (init_flag) {  // initalization happened from Kilogrid
        ...
        if (received_grid_msg_flag) { //if received message from kilogrid
            ...
            if (kilogrid_commitment ==
                1) {  //if it option A- red recived from Kilogrid  (1 for a normal red tile and 6 for border red tile)
                ...
                set_color(RGB(1, 1, 0));
                       ...
                       2) {  //if its option b- Blue received from Kilogrid (3 for a normal blue tile and 9 for border blue tile)
                ...
                set_color(RGB(0, 1, 1));
                       ...
                       3) {  //if its option b- Blue received from Kilogrid (3 for a normal blue tile and 9 for border blue tile)
//////////                printf("%d  changes commitment to 3 from kilogrid\n", kilogrid_commitment);
                ...
                set_color(RGB(0, 0, 1));
                       ...
                       4) {  //if its option b- Blue received from Kilogrid (3 for a normal blue tile and 9 for border blue tile)
                ...
                if (foundmodules[msg->data[0]][msg->data[1]] == 0) { //if tile not counted previously
                    ...
                    if ((received_option_kilogrid == 1) &&
                        ...
                        tiles_of_my_option += 1;
                               ...
                               currentopinion == 2) { //if I am blue and I receive blue from Kilogrid
                        tiles_of_my_option += 1;

                    } else if ((received_option_kilogrid == 3) &&
                               currentopinion == 3) { //if I am blue and I receive blue from Kilogrid
                        tiles_of_my_option += 1;

                    } else if ((received_option_kilogrid == 4) &&
                               currentopinion == 4) { //if I am blue and I receive blue from Kilogrid
                        tiles_of_my_option += 1;
...
message_t *message_tx() {
    if (broadcast_msg &&
        message.data[1] != UNCOMMITTED) { //if broadcast message flag is set to true (only in dissem state function)
        // printf("broadcast %d \n", message.data[1] );
