...
def gillespieStep(state, N, gammas, alphas, rhos, sigmas, vectorsOfChange, timeLeft):
    ...
    for i in range(0, numOptions):  # PAY ATTENTION THAT i IS NOT THE CORRECT state[i] BUT IT MUST BE state[i+1], WHILE WORKS FOR gammas[i], alphas[i], rhos[i]
        ...
        #############Noisyyyy Cross-inhibition#############
        # [ U, AD, BD, AE, BE ]
        if ((state[1] + state[2])) <= 1:
            probabilitiesOfChange.append(0)
        else:
            probabilitiesOfChange.append(
                state[1] * state[2] * ((noisevalue) / (qualities[i] * t_d)) / ((state[1] + state[2]) - 1))

        ############Noisyyyy Recruitment###################
        # probabilitiesOfChange.append(state[0] * state[i + 1] * rhos[i] / (N - 1))  # U + A -> A + A
        if ((state[1] + state[2])) == 0:
            probabilitiesOfChange.append(0)
        else:
            probabilitiesOfChange.append((state[0] * state[i + 1] * ((noisevalue) / t_u)) / (state[1] + state[2]))
        ...
        if ((state[1] + state[2])) <= 1:
            ...
            probabilitiesOfChange.append(
                state[i + 1] * state[i + 1] * ((noisevalue) / (qualities[i] * t_d)) / ((state[1] + state[2]) - 1))

       # probabilitiesOfChange.append(state[i + 1] * ((noisevalue) / (qualities[i] * t_d)))  # AD->BE and BD->AE
       # probabilitiesOfChange.append(state[i + 1] * ((noisevalue) / (qualities[i] * t_d)))  # AD->AE and BD->BE
       # probabilitiesOfChange.append(state[0] * ((noisevalue) / (t_u)))  # U->AE and U->BE
...
def runGillespie(state, T, N, gammas, alphas, rhos, sigmas, rnd_seed, finalStateFile, temporalEvolution, plot_evo,
    ...
    for i in [1, 2]:
        if (i == 1):  # pop A
            ...
            expplorationend = [0, 1, 0, -1, 0] #for exploration to dissem

            noisecrossinhib = [0, -1, 0, 1, 0] # for noisy cross inhibition
            noiserecruitment = [-1, 0, 0, 0, 1] # for noisy recruitment
            noisenocross = [1, -1, 0, 0, 0] # for noisy no cross-inhibition recruitment
        ...
        if (i == 2):  # pop B
            ...
            noisecrossinhib = [0, 0, -1, 0, 1]
            noiserecruitment = [-1, 0, 0, 1, 0]
            noisenocross = [1, 0, -1, 0, 0]
        ...
        vectorsOfChange.append(noisecrossinhib)
        vectorsOfChange.append(noiserecruitment)
        vectorsOfChange.append(noisenocross)
    ...
    while t < T:
        ...
        spd[int(previous_state[1]) + int(previous_state[3])][
            int(previous_state[2]) + int(previous_state[4])] += time_step
...
if __name__ == '__main__':
   ...
   #### nr2 = [float(sys.argv[9]), 1 - float(sys.argv[9])]
    ...
    for valueA in qa:
        finalStateFile = 'none'
        finalStateFile = 'data/fs_0.00q_n_' + str(noisevalue) + '_h_' + str(N) + '_vA_' + str(qa) +'tutdte'+str(t_u)+str(t_d)+str(t_e)+ '_r1.txt'
        ...
        for valueB in valuesB:
            ...
            for i in range(0, repetitions):
               ...
               # state = [0,N/4,N/4,N/4,N/4]
                ...
                #temporalEvolution = 'evo'+'/popevoci_' + str(state) + str(t_u) + "td" + str(t_d) + "te" + str(t_e) + "_" + str(
                #    noisevalue) + "noise_" + "500ksteps_" + str(repetitions) + "runs_" + str(N) + "agents_" + str(
                #    qualities) + str(quorum) + '/' + 'evo-N_' + str(noisevalue)+"_"+str(qa)+"_" +str(i) + str(rnd_seed) + '.txt'
                rnd_seed = np.random.randint(4325344)
                # rnd_seed = 43431
                print(rnd_seed)
                # S = N
...
outputname =  "/home/rzakir/Programs/gillespieaccnew/matrix/outputoftu"  + str(state) + str(t_u) + "td" + str(t_d) + "te" + str(t_e) + "_" + str(noisevalue) + "noise_" + "500ksteps_" + str(repetitions) + "runs_" + str(N) + "agents" + str(qualities)+"_"+str(randno)+str(randno2)+str(randno3)+str(randno4)+ ".txt"
